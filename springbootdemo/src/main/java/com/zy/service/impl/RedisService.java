...
public class RedisService {
	...
	@Autowired
	public RedisService(ShardedJedisPool jedisPool) {
	...
	/**
	 * 批量设置到hash数据结果，采用byte类型存储， 取的时候得注意数据类型转换（例如：Map
	 * <key,value>中put数据时key的类型为String，那么get的时候需严格用String类型,否则get时会得不到你想要的）
	 * 
	 * @param mapKey
	 * @param data
	 */
	 ...
	 * @param member set中成员
	 */
	public void removeSetMember(String setKey, String member) {
		ShardedJedis conn = getConnection();
		if (conn == null) {
			return;
		}
		try {
			conn.srem(setKey, member);
			returnConnection(conn);
		} catch (Exception e) {
			returnBorkenConnection(conn);
		}
	 ...
	 * @param setKey redis的key
	 */
	public boolean isSetMember(String setKey, String member) {
		ShardedJedis conn = getConnection();
		if (conn == null) {
			return false;
		}
		...
		try {
			isMember = conn.sismember(setKey, member);
			returnConnection(conn);
		} catch (Exception e) {
			returnBorkenConnection(conn);
		}
	...
	public Long ttl(Object key) {
		...
		try {
			byte[] b = serializable(key);
			Long result = conn.ttl(b);
			returnConnection(conn);
			return result;
		} catch (Exception e) {
			returnBorkenConnection(conn);
